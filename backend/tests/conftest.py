...
Baseten, OpenAI (the squack agent, `tower.llm.get_agent_llm`) and ElevenLabs: slow, billed, and
flaky, because a model's wording changes between runs.
Without keys the code takes its deterministic local paths, which is what the tests pin.
"""
import pytest

PAID = ("BASETEN_API_KEY", "OPENAI_API_KEY", "ELEVENLABS_API_KEY", "ASR_MODEL_URL",
        "ASR_STOCK_MODEL_URL", "CHECKER_MODEL_URL")
