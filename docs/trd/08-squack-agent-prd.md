...
- **One loop.** `backend/agent/loop.py` generalises `world_agent._llm_agent`: same `LLM.chat(messages, tools=...)`. **Provider, as built (Sept 20):** this conversational agent runs on OpenAI, the one deliberate exception to hard rule 5. `tower.llm.get_agent_llm()` prefers `OPENAI_API_KEY` with `SQUACK_MODEL` (default `gpt-4o-mini`), falls back to Baseten's `RESOLVER_MODEL` when only `BASETEN_API_KEY` is set, and to the keyword router with neither. Both fine-tuned models (the tuned Whisper and the readback checker), the resolver, the interpreter and the extractor fallback are unchanged and still on Baseten. `world_agent.py` becomes a thin caller of the new loop so the headset channel and the bar share one brain.
...
- **Without a key.** (As built: without `OPENAI_API_KEY` and `BASETEN_API_KEY`.) `backend/agent/router.py`: a keyword router for rungs (a) and (b) only, extended from `world_agent._keyword_agent`: focus, follow, speed, voice, line view, load, disrupt, sliders, start, pause, reset, describe. Query, explain and sim rungs return a `text` card: "needs BASETEN_API_KEY". Say so in the README row.
