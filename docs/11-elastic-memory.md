...
- **The container that wrote this could not reach the cluster** (egress policy), so the live path was verified only by unit tests against the fake. `tools/elastic_check.py` is the first thing to run on a laptop after merging. If a search returns `BAD`, look at the query in `memory.py` and the mapping in Kibana Dev Tools with `GET tower-*/_mapping`.
