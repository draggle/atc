...
| Instruction cards | bottom right, fixed height, scrolls | one card per instruction: tag, phrase, reason, confidence, urgency; later flights collapsed; done log | hold Space and say it, let squack say it, click to focus |
...
| Analytics (was Scoreboard) | right rail, collapsed by default | 16 measured numbers; the collapsed bar shows losses and conflicts predicted | click to open; two example questions send themselves to squack |
