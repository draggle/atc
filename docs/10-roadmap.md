...
**The speed buttons looked dead with voice on, fixed.** The self-slowing clock held 1x for as long as *any* card was waiting to be said. Since the controller may now ignore a card or answer it with something else, a card could wait for ever, and 5x, 20x and 60x did nothing, with only a small chip to say why. Now a card holds the clock for `CARD_HOLD_S` (12 real seconds) from the moment it first needs saying, which is time to see it and key the mic; after that the chosen speed is back and the card waits on the list. **Pressing any speed button always takes effect at once** (`set_speed` releases whatever is holding). Only an exchange really in progress still runs at 1x (`World.on_the_radio`: key down, Tower speaking, an instruction out with its readback not in, a pilot about to key up), because speech takes real time. The radar frame says why and for how long (`clock_why`, `clock_hold_s`) and the chip reads "1x: new card · 20x in 9 s (or press a speed)" or "1x: on the radio". Watched live at 20x: fast, 1x for twelve seconds when DAL789's conflict card appeared, then 20x again with nobody having said it.

Then tried on the live Europe sky (62 aircraft, 60x), where it still felt slow after each instruction. Two more bounds. (1) "An instruction is out and its readback is not in" lasted the clearance's whole 25 s timeout, so a readback Tower could not match (EZY35JL through the radio) was 25 s at 1x and then the alert about it held the clock again: it now holds for `EXCHANGE_HOLD_S`, 10 real seconds, plus for as long as a pilot's reply is actually being made or heard. (2) In a busy sky cards arrive all the time and each opened its own window, back to back. Now one window (`CARD_HOLD_S`, 8 s) is shared by whatever turns up during it, and routine cards do not open another for `CARD_HOLD_COOLDOWN_S` (20 s). What follows from the controller's own exchange (back on course, a wrong readback, an emergency) always gets its window. Not touched, on purpose, this late: with a disruption active the periodic replan is every 15 sim seconds, which at 60x is every tick, and with 60 aircraft that is most of the tick.
