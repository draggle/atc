...
/** The setup panel's own scenario: generated by the backend from these three numbers. */
const CUSTOM = "custom";
const PACES = [
          ...
          {source === "sim" ? (
            <div className="p-5 flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                ...
                {sims.length > 0 && (
                  <button onClick={() => setScenario(CUSTOM)} className={pick(scenario === CUSTOM)}>
                    <div className="flex items-baseline justify-between gap-3">
                      <span className="text-sm font-medium">Custom</span>
                      <span className="text-xs font-mono text-muted">{flights} flights</span>
                    </div>
                    <p className="text-xs text-muted mt-0.5">Your own sky: how many aircraft, and how fast they arrive.</p>
                  </button>
                )}
              </div>
              {scenario === CUSTOM ? (
                <div className="flex flex-col gap-4">
                  <label className="flex flex-col gap-2">
                    <span className="flex items-baseline justify-between">
                      ...
                      <span className="font-mono text-sm text-fg">{flights}</span>
                    </span>
                    <input type="range" min={2} max={80} step={1} value={flights} onChange={(e) => setFlights(Number(e.target.value))} />
                    <span className="flex justify-between text-[10px] font-mono text-muted"><span>2</span><span>20</span><span>40</span><span>60</span><span>80</span></span>
                  </label>
                  <div className="flex flex-col gap-2">
                    <span className="eyebrow">How fast they arrive</span>
                    <div className="flex items-center gap-3">
                      <div className="flex rounded-md border border-line overflow-hidden w-fit">
                        {PACES.map((p) => (
                          <button key={p.key} onClick={() => setPace(p.key)} className={seg(pace === p.key)}>{p.label}</button>
                        ))}
                      </div>
                      <span className="text-xs text-muted">{PACES.find((p) => p.key === pace)?.every}, three already entering at Start</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2">
                    <span className="eyebrow">Which draw</span>
                    <div className="flex items-center gap-3">
                      <button onClick={() => setSeed((n) => n + 1)} className="px-3 py-1 rounded-md border border-line bg-panel-2/70 text-sm text-muted hover:text-fg">Shuffle</button>
                      <span className="text-xs text-muted">Draw <span className="font-mono text-fg">{seed}</span>. The same three settings always give the same sky, so a good one can be shown again.</span>
                    </div>
                  </div>
                </div>
              ) : (
